...
public class GoodControllerTest {
    ...
    @Test
    @WithMockUser(username = "jimmy_mogilev@yopmail.com", password = "P@ssword1")
    void givenId_whenGetExistingProduct_thenStatus200andProductReturned() throws Exception {
        long id = createTestProduct("Phone", BigDecimal.valueOf(4.2), 3L, "This is a phone",
                "jimmy_mogilev@yopmail.com").getId();
    ...
    void givenError_whenGetNotExistingProduct_thenStatus404NotFound() throws Exception {
        ...
        String error = "Good with id " + (id + 1) + " not found";
    ...
    @Test
    @WithMockUser(username = "den_mogilev@yopmail.com", password = "1234")
    void givenAllGoods_whenIAmAdmin_thenStatus200() throws Exception {
    ...
    void givenAllGoods_whenIAmAdminAndHaveSearchAndSortParameters_thenStatus200() throws Exception {
        List<GoodAdminViewDto> goods = goodService.getAllForAdmin("title",
                "b", "id", "desc", 10, 1);

        mockMvc.perform(
                        get("http://localhost:8081/goods/forAdmin?pageSize=10&pageNumber=1&sortField=id&sortDirection=desc&searchField=title&parameter=b"))
                .andExpect(status().isOk())
                .andExpect(content().json(objectMapper.writeValueAsString(goods)));
    }
    ...
    @Test
    @WithMockUser(username = "den_mogilev@yopmail.com", password = "1234")
    public void givenError_whenDeleteNotExistingProduct_thenStatus404NotFound() throws Exception {
        long id = goodService.getTotalAmount();

        String error = "Good with id " + (id + 1) + " not found";

        mockMvc.perform(
                        delete("http://localhost:8081/goods/forAdmin/{id}", id + 1))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.info").value(error));
    }
